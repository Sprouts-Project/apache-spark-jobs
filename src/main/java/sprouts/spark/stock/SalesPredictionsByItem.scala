...
case class ItemVectorByItem(label: Double, features: SparseVector)
case class SaleByItem(month: Int, year: Int, itemId:Int, itemName:String, sales: Int)

object SalesPredictionsByItem extends SparkJob {
...
AND order_.date < DATE_FORMAT(NOW() ,'%Y-%m-01')) AS data
    ...
    val toPredict = sqlContext.createDataFrame(
      sc
        ...
        .map {
          x =>
            ItemVectorByItem(0.0, new SparseVector(3, Array(0, 1, 2), Array(x._1.toDouble, x._2.toDouble,x._3.toDouble)))
        }))
